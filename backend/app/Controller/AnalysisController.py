...
class AnalysisController:
    ...
    def getDataTracker(datetime_start, datetime_end):
        ...
        for instrument_name in coins_list:
            docs = list(
                db_tracker[instrument_name].find(
                    {"_id": {"$gte": datetime_start, "$lt": datetime_end}}, filter_query
                )
            )
    ...
    def getDataMarket(datetime_start, datetime_end):
        ...
        for instrument_name in coins_list:
            docs = list(
                db_market[instrument_name].find(
                    {"_id": {"$gte": datetime_start, "$lt": datetime_end}}, filter_query
                )
            )
            dict_[instrument_name] = docs
    ...
    def try_different_timeframe(
        ...
        for different_timeframe in rules_for_nan[timeframe]:
            ...
            x_time_ago = datetime.fromisoformat(start_timestamp) - timedelta(
                minutes=minutes
            )
            ...
            docs = list(
                db_tracker[coin].find(
                    {"_id": {"$gte": timestamp_start, "$lt": timestamp_end}},
                    {"_id": 0, "price": 1, vol_field: 1, buy_vol_field: 1},
                )
            )
            price_list = [
                obj["price"] for obj in docs if "price" in obj and obj["price"] != None
            ]
            vol_list = [
                obj[vol_field]
                for obj in docs
                if vol_field in obj and obj[vol_field] != None
            ]
            buy_vol_list = [
                obj[buy_vol_field]
                for obj in docs
                if buy_vol_field in obj and obj[buy_vol_field] != None
            ]
        ...
        msg = (
            f"-{start_timestamp} - {coin}: CANNOT REPLACE ALL NAN VALUES - {event_key}"
        )
        log_nan_replaced += [msg]
        return docs, log_nan_replaced
    ...
    def get_price_changes(request):
        ...
        rules_for_nan = {
            "info_5m": [7, 8, 9],
            "info_15m": [13, 17, 11, 19],
            "info_30m": [28, 32, 26, 34],
            "info_1h": [0.5, 1.5, 2, 2.5],
            "info_2h": [1.5, 2.5, 1, 3, 3.5],
            "info_3h": [2, 4, 5],  # hours
            "info_6h": [5, 7, 4, 8, 9, 10],
            "info_12h": [11, 13, 10, 14, 15, 16],
            "info_18h": [17, 19, 16, 20, 16, 21],  # hours
            "info_1d": [0.85, 1.15, 0.7, 1.3, 1.5, 1.7, 2],
            "info_2d": [1.85, 2.15, 1.7, 2.3, 1.5, 2.5, 2.75, 3],
            "info_5d": [4.5, 5.5, 4, 6, 3.5, 6.5, 3, 7],  # days
            "info_3d": [2.85, 3.15, 2.7, 3.3, 2.5, 3.5, 3.75, 2.25, 4, 4.5, 5],
            "info_7d": [6.5, 7.5, 6, 7, 5, 9],
        }  # days
        ...
        for event_key in request:
            ...
            for coin in request[event_key]:
                ...
                for start_timestamp in request[event_key][coin]:
                    ...
                    obj = list(
                        db_tracker[coin].find(
                            {"_id": start_timestamp}, {"_id": 0, "price": 1}
                        )
                    )
                    ...
                    for timeframe in list_timeframes:
                        ...
                        # set timestamp start and timestamp end for db query
                        x_time_ago = datetime.fromisoformat(
                            start_timestamp
                        ) - timedelta(minutes=list_timeframes[timeframe])
                        timestamp_start = (
                            x_time_ago - timedelta(minutes=5)
                        ).isoformat()
                        ...
                        price_list = [
                            obj["price"]
                            for obj in docs
                            if "price" in obj and obj["price"] != None
                        ]
                        vol_list = [
                            obj[vol_field]
                            for obj in docs
                        ...
                        if (
                            len(price_list) == 0
                            and len(vol_list) == 0
                            and len(buy_vol_list) == 0
                        ):
                            docs, log_nan_replaced = (
                                AnalysisController.try_different_timeframe(
                                    start_timestamp,
                                    timeframe,
                                    rules_for_nan,
                                    db_tracker,
                        ...
                        if len(docs) > 0:
                            if len(price_list) > 0:
                                price_change = round_(
                                    (price_start_timestamp - np.mean(price_list))
                                    / np.mean(price_list),
                                    4,
                                )
                            else:
                            ...
                            response[event_key][coin][start_timestamp][timeframe] = (
                                price_change,
                                vol_change,
                                buy_vol_change,
                            )
    ...
    def get_timeseries(request):
        ...
        for coin in coins:
            ...
            for start_timestamp in start_timestamp_list:
                ...
                timeseries_start = datetime.fromisoformat(start_timestamp).replace(
                ...
                datetime_end = (
                    ...
                    + timedelta(minutes=check_future)
                )
        ...
        total_timeseries_expected = sum(
            [
                1
                for coin in request["info"][event_key]
                for start_timestamp in request["info"][event_key][coin]
            ]
        )
        total_timeseries_downloaded = sum(
            [1 for coin in response["data"] for _ in response["data"][coin]]
        )

        if (
            response["msg"]
            == "WARNING: Request too big. Not all data have been downloaded, retry..."
        ):
            response["msg"] = (
                f"WARNING: Request too big. Not all data have been downloaded. {total_timeseries_downloaded}/{total_timeseries_expected}. Retry..."
            )
        elif total_timeseries_expected != total_timeseries_downloaded:
            response["msg"] = (
                f"WARNING: timeseries downloaded: {total_timeseries_downloaded}/{total_timeseries_expected}"
            )
        else:
            response["msg"] = (
    ...
    def getTimeseries(request):
        ...
        for coin in coins:
            ...
            for event in events:
                if datetime.fromisoformat(event["event"]) > datetime.fromisoformat(
                    request["last_timestamp"][coin]
                ):
                    if n_events <= n_event_limit:
                        ...
                        if not check_past:
                            ...
                            timeseries_start = datetime.fromisoformat(
                                event["event"]
                            ).replace(second=0, microsecond=0)
                            # datetime_start is the beginning start from which retrieving the observation
                            datetime_start = timeseries_start - timedelta(
                                minutes=check_past
                            )
                            datetime_end = (
                                timeseries_start
                                + timedelta(minutes=timeframe)
                                + timedelta(minutes=timeframe / 3)
                            )
                        ...
                        response["data"][coin][event["event"]] = {
                            "data": docs,
                            "statistics": {
                                "mean": event["mean"],
                                "std": event["std"],
        ...
        total_timeseries_expected = sum(
            [
                1
                for coin in request["info"]
                for event in request["info"][coin]
                if datetime.fromisoformat(event["event"])
                > datetime.fromisoformat(request["last_timestamp"][coin])
            ]
        )
        total_timeseries_downloaded = sum(
            [1 for coin in response["data"] for _ in response["data"][coin]]
        ...
        if len(key_set) == len(request):
            ...
            for user in request:
                if not set(keys_to_check).issubset(request[user]):
                    ...
                    user_error = user
                    msg = (
                        f"Configuration for {user} is incorrect. some keys are missing"
                    )
    ...
    def get_crypto_timeseries(request):
        ...
        for coin in request:
            ...
            timeseries = list(
                db_tracker[coin].find(
                    {"_id": {"$gte": start_timestamp, "$lt": end_timestamp}},
                    {"price": 1},
                )
            )
        ...
        response = {
            "data": crypto_timeseries,
            "all_data_downloaded": all_data_downloaded,
        }
