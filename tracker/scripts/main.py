...
def main(
    ...
    while True:
        ...
        if now.second == SECOND_START_TRACKING:
            t1 = time()
            TrackerController.start_tracking(
                best_x_coins,
                coins_list,
                volume_standings=volume_standings,
                db_trades=db_trades,
            ...
            FIRST_RUN = False
            time_delta = (datetime.now() + timedelta(minutes=1)).replace(
                second=SECOND_START_TRACKING, microsecond=0
            ) - datetime.now()
...
if __name__ == "__main__":
    ...
    main(
        ...
        db_benchmark=db_benchmark,
        db_logger=db_logger,
        db_volume_standings=db_volume_standings,
        logger=logger,
        FIRST_RUN=FIRST_RUN,
        SECOND_START_TRACKING=SECOND_START_TRACKING,
