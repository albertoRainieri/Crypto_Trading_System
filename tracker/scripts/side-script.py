...
def main(client, logger, db_logger, user_configuration):
    ...
    while True:
        ...
        if minute == 59 and second == 15 and hour == 23:
            ...
            logger.info(
                "Performance has been updated from 'TradingController.checkPerformance'"
            )
            sleep(0.2)
        ...
        if minute == 59 and second == 18 and hour == 23:
            ...
            logger.info(
                "list of instruments updated from 'BinanceController.get_exchange_info'"
            )

        if minute == 59 and second == 30 and hour == 23:
            Benchmark.computeVolumeAverage(client=client)
            logger.info(
                "volumes have been updated from 'Benchmark.computeVolumeAverage'"
            )
