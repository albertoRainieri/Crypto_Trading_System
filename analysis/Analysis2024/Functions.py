...
import shutil
import re
from Analysis2024.Helpers import (
    round_,
    get_volume_standings,
    data_preparation,
    load_data,
    get_benchmark_info,
    get_dynamic_volatility,
    getnNewInfoForVolatilityGrouped,
    riskmanagement_data_preparation,
)
from Analysis2024.Helpers import (
    get_substring_between,
    load_analysis_json_info,
    updateAnalysisJson,
    pooled_standard_deviation,
    getsubstring_fromkey,
    get_currency_coin,
    load_volume_standings,
    filter_xth_percentile,
)
import calendar
...
def total_function_multiprocessing(
    event_keys,
    analysis_timeframe,
    n_processes,
    KEEP_PRODUCTION_ANALYSIS,
    analysis_name,
    ...
    if KEEP_PRODUCTION_ANALYSIS:
        ...
        analysis_json_path = ROOT_PATH + f"/analysis_json/{analysis_name}.json"

    start_interval, end_interval = load_analysis_json_info(
        analysis_json_path,
    ...
    data = load_data(
        start_interval=datetime.fromisoformat(start_interval), end_interval=end_interval
    )
    ...
    pool.starmap(
        wrap_analyze_events_multiprocessing,
        [
            (
                arg,
                arg_i,
                event_keys,
                volume_standings_full,
                end_interval_analysis,
                lock,
                shared_data,
            )
...
def wrap_analyze_events_multiprocessing(
    data,
    data_i,
    event_keys,
    volume_standings_full,
    ...
    for event_key in event_keys:
        vol, vol_value, buy_vol, buy_vol_value, timeframe, lvl = getsubstring_fromkey(
            event_key
        )
        if data_i == 1:
            iteration += 1
    ...
    print(
        f"size of shared_data_value before completion of {data_i}: {size_shared_data_value} Mb; Number of events saved: {n_events}"
    )
            ...
            if datetime_obs < end_interval_analysis:
                if buy_vol in obs and obs[buy_vol] != None:
                    ...
                    if obs[vol] > vol_value and datetime_obs > limit_window:
                        ...
                        # if buy_vol is 0, it means that buy_vol check is skipped
                        if buy_vol_value != 0:
                        ...
                        volume_standings = int(
                            get_volume_standings(
                                volume_standings_full, obs["_id"], coin
                            )
                        )
                        ...
                        for obs_event, obs_i in zip(
                            data[coin][index : index + timeframe], range(timeframe)
                        ):
                            ...
                            actual_datetime = datetime.fromisoformat(
                                data[coin][index + obs_i]["_id"]
                            )
                            if actual_datetime - datetime.fromisoformat(
                                data[coin][index + obs_i - 1]["_id"]
                            ) <= timedelta(minutes=10, seconds=10):

                                change = (
                                    obs_event["price"] - initial_price
                                ) / initial_price
                        ...
                        complete_info[coin].append(
                            {
                                "event": timestamp_event_triggered,
                                "mean": round_(np.mean(event_price_changes), 4),
                                "std": round(np.std(event_price_changes, ddof=1), 4),
                                "max": max_price_change,
                                "min": min_price_change,
                                "lvl": volume_standings,
                            }
                        )
            else:
...
def download_show_output(
    minimum_event_number,
    mean_threshold,
    frequency_threshold,
    early_validation=False,
    std_multiplier=3,
    start_analysis=datetime(2023, 1, 1),
    file_paths=None,
    ...
    for file_path in file_paths:
        ...
        for event_key in list(analysis_json.keys()):
            ...
            vol, vol_value, buy_vol, buy_vol_value, timeframe, lvl = (
            ...
            if (
                event_key not in first_event
                and len(analysis_json[event_key]["info"]) != 0
            ):
                random_coin = list(analysis_json[event_key]["info"].keys())[0]
                first_event[event_key] = datetime.fromisoformat(
                    analysis_json[event_key]["info"][random_coin][0]["event"]
                )
            ...
            for coin in analysis_json[event_key]["info"]:
                for event in analysis_json[event_key]["info"][coin]:
                    ...
                    if (
                        early_validation
                        and datetime.fromisoformat(event["event"]) > early_validation
                    ):
                        continue
                    elif (
                        start_analysis
                        and datetime.fromisoformat(event["event"]) < start_analysis
                    ):
                        continue
                    else:
                        ...
                        first_event[event_key] = min(
                            datetime.fromisoformat(event["event"]),
                            first_event[event_key],
                        )
                        last_event[event_key] = max(
                            datetime.fromisoformat(event["event"]),
                            last_event[event_key],
                        )

            if DELETE_99_PERCENTILE:
                filtered_info_by_coin_list, events_discarded = filter_xth_percentile(
                    info_by_coin_list, filter_field, xth_percentile
                )
            ...
            if len(mean_by_coin_list) > 0:
                ...
                median_max_by_coin = round_(
                    np.median(np.array(max_by_coin_list)) * 100, 2
                )
                median_min_by_coin = round_(
                    np.median(np.array(min_by_coin_list)) * 100, 2
                )
                max_by_coin = round_(np.mean(np.array(max_by_coin_list)) * 100, 2)
                min_by_coin = round_(np.mean(np.array(min_by_coin_list)) * 100, 2)
                std_by_coin = round_(
                    pooled_standard_deviation(
                        np.array(std_by_coin_list), sample_size=int(timeframe)
                    )
                    * 100,
                    2,
                )
                avg_standings_by_coin = round_(
                    np.mean(np.array(standings_by_coin_list)), 2
                )
                ...
                if event_key not in output:
                    ...
                    output[event_key] = {
                        "mean": [(mean_by_coin, len(mean_by_coin_list))],
                        ...
                        "standings": [
                            (avg_standings_by_coin, len(standings_by_coin_list))
                        ],
                        "n_coins": list_coins,
                        "n_events": n_events,
                    }
                    # update 'info' by adding "volatility" variable
                    complete_info[event_key] = {
                        "info": {},
                        "n_coins": list_coins,
                        "events": n_events,
                    }
                    ...
                    # update output
                    output[event_key]["mean"].append(
                        (mean_by_coin, len(mean_by_coin_list))
                    )
                    output[event_key]["std"].append(
                        (std_by_coin, len(std_by_coin_list))
                    )
                    output[event_key]["max"].append(
                        (max_by_coin, len(max_by_coin_list))
                    )
                    output[event_key]["min"].append(
                        (min_by_coin, len(min_by_coin_list))
                    )
                    output[event_key]["median_max"].append(
                        (median_max_by_coin, len(mean_by_coin_list))
                    )
                    output[event_key]["median_min"].append(
                        (median_min_by_coin, len(mean_by_coin_list))
                    )
                    output[event_key]["standings"].append(
                        (avg_standings_by_coin, len(standings_by_coin_list))
                    )
        ...
        for event_key in output:
            mean = sum([item[0] * item[1] for item in output[event_key]["mean"]]) / sum(
                [item[1] for item in output[event_key]["mean"]]
            )
            std = sum([item[0] * item[1] for item in output[event_key]["std"]]) / sum(
                [item[1] for item in output[event_key]["std"]]
            )
            median_max = sum(
                [item[0] * item[1] for item in output[event_key]["median_max"]]
            ) / sum([item[1] for item in output[event_key]["median_max"]])
            median_min = sum(
                [item[0] * item[1] for item in output[event_key]["median_min"]]
            ) / sum([item[1] for item in output[event_key]["median_min"]])
            max_ = sum([item[0] * item[1] for item in output[event_key]["max"]]) / sum(
                [item[1] for item in output[event_key]["max"]]
            )
            min_ = sum([item[0] * item[1] for item in output[event_key]["min"]]) / sum(
                [item[1] for item in output[event_key]["min"]]
            )
            avg_standings = sum(
                [item[0] * item[1] for item in output[event_key]["standings"]]
            ) / sum([item[1] for item in output[event_key]["standings"]])
            ...
            if time_interval_analysis != 0:
                event_frequency_month = round_(
                    (n_events / time_interval_analysis) * 30, 2
                )

            if (
                output[event_key]["n_events"] >= minimum_event_number
                and mean > mean_threshold
                and mean <= std_multiplier * std
                and event_frequency_month >= frequency_threshold
            ):
                ...
                output[event_key]["upper_bound"] = (
                    output[event_key]["mean"] + output[event_key]["std"]
                )
                output[event_key]["lower_bound"] = (
                    output[event_key]["mean"] - output[event_key]["std"]
                )
                output[event_key]["n_coins"] = len(set(output[event_key]["n_coins"]))
                output[event_key]["frequency/month"] = event_frequency_month
                complete_info[event_key]["n_coins"] = len(
                    set(complete_info[event_key]["n_coins"])
                )
