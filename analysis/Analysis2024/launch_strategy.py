...
info_strategy_list_legend = {
        ...
        'limit': [0.25], #get the window of price change (from 0 to 1) (e.g. 0.15 check only the orders whose level is within 15% price change from current price)
        'price_change_jump': [0.025], #range of price change (used in get_price_levels for bid/ask_order_distribution)
        'max_limit': [0.05],  #what is the max change, beyond that I won't consider triggers ---> ( max_price - initial_price ) / initial_price
        ...
        'distance_jump_to_current_price': [1], # minimum distance from jump
...
if __name__ == '__main__':
    ...
    analysis_name = "analysis-official-january-april-2025"
    analysis_name = None
    ...
    output, complete_info = get_analysis(analysis_name)
    #print(f'complete_info: {complete_info}')
    event_keys_filter = []
    check_past=60 #minutes before event trigger
    ...
    for info_strategy_i, info_strategy in enumerate(info_strategy_list):
        ...
        t1 = time()
        get_timeseries(complete_info, check_past=check_past, check_future=check_future, info_strategy=info_strategy,
                       save_plot=save_plot, analyze=analyze, n_processes=8, skip_if_strategy_exists=False, analysis_name=analysis_name)
        t2 = time()
        seconds_spent = t2 - t1
